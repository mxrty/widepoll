export const CREATE_POST = "CREATE_POST";
export const FETCH_POST = "FETCH_POST";
export const FETCH_POSTS = "FETCH_POSTS";
export const EDIT_POST = "EDIT_POST";
export const DELETE_POST = "DELETE_POST";
export const LIKE_POST = "LIKE_POST";
export const UNLIKE_POST = "UNLIKE_POST";

export const CREATE_DOMAIN = "CREATE_DOMAIN";
export const FETCH_DOMAIN = "FETCH_DOMAIN";
export const FETCH_DOMAINS = "FETCH_DOMAINS";
export const EDIT_DOMAIN = "EDIT_DOMAIN";
export const DELETE_DOMAIN = "DELETE_DOMAIN";

export const SIGN_IN = "SIGN_IN";
export const SIGN_OUT = "SIGN_OUT";
export const REGISTER = "REGISTER";
export const BECOME_REP = "BECOME_REP";
export const FETCH_USER = "FETCH_USER";
export const FOLLOW_REP = "FOLLOW_REP";
export const UPDATE_REP_RANKING = "UPDATE_REP_RANKING";

export const CREATE_COMMENT = "CREATE_COMMENT";
export const FETCH_COMMENTS = "FETCH_COMMENTS";
export const LIKE_COMMENT = "LIKE_COMMENT";
export const UNLIKE_COMMENT = "UNLIKE_COMMENT";

export const CREATE_SOLUTION = "CREATE_SOLUTION";
export const FETCH_SOLUTION = "FETCH_SOLUTION";
export const FETCH_SOLUTIONS = "FETCH_SOLUTIONS";
export const LIKE_SOLUTION = "LIKE_SOLUTION";
export const UNLIKE_SOLUTION = "UNLIKE_SOLUTION";

export const CREATE_POLL = "CREATE_POLL";
export const FETCH_POLL = "FETCH_POLL";

export const CREATE_SENTIMENT = "CREATE_SENTIMENT";
export const FETCH_SENTIMENTS = "FETCH_SENTIMENTS";

export const FETCH_PENDING_VOTES = "FETCH_PENDING_VOTES";
export const DELETE_PENDING_VOTE = "DELETE_PENDING_VOTE";
